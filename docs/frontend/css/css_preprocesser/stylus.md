...
//--------------------------------
// 范围选择器 实际上就是按照层级吧选择器截取出来  ^[1..-2] 为div p 从第二个到倒数第二个
...
#### 4. 运算符，很接近JavaScript  但也引入了部分python风格的运算符
...
sort(list, fn = null) // 定义一个排序方法 
  ...
  // 冒泡排序
  for i in 1...length(list) // 数组长度 左闭右开
    for j in 0...i // 内循环
