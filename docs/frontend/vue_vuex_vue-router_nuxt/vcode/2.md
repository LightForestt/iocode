...
> 引言：试想一个场景，在开发项目过程中往往会用到 Vue 的全局 api - mixin，很神奇的是全局混入的属性我们可以在任意一个组件中通过`this.propName`进行访问，就像这些属性定义到了子组件中，那么 vue 是如何达到这种效果的呢？合并！
> 如果组件内部混入了一些属性要如何和组件本身的 options 进行合并呢（局部混入）？同样也是通过合并。
